  ...
  const requestOtp = async () => {
    ...
    try {
      const { data, error: fnErr } = await supabase.functions.invoke(FN_PASSWORD_RESET_REQUEST, {
        body: { email: email.trim().toLowerCase(), account_type: "admin" },
      });
      if (fnErr) {
  ...
  const verifyOtp = async () => {
    ...
    try {
      const { data, error: fnErr } = await supabase.functions.invoke(FN_PASSWORD_RESET_VERIFY, {
        body: { email: email.trim().toLowerCase(), otp: otp.trim(), account_type: "admin" },
      });
      if (fnErr) {
        setError(fnErr.message);
        return;
    ...
    <AuthSplitLayout>
      <div className="auth-card">
        ...
        <form onSubmit={onSubmit}>
          ...
          {error ? <div style={{ color: "#dc2626", fontSize: 14, marginTop: 10 }}>{error}</div> : null}
