...
export function SoundEnablePrompt() {
  ...
  const onEnable = useCallback(async () => {
    setBusy(true);
    setShowErr(false);
    // Ask permission for browser notifications (optional), so alerts can still show when tab is in background.
    try {
  ...
  return createPortal(
    <div className="sound-enable-prompt" role="dialog" aria-labelledby="sound-enable-title">
      ...
      <p className="sound-enable-prompt__text">
        {enabled
          ? "Your browser blocked autoplay. Tap below once so reservation alerts can play sound."
          : "Tap below so reservations and other alerts can play a sound."}
      </p>
